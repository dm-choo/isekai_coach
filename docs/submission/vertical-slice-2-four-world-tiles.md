...
- 각 통로는 4개의 구간을 가진다.
- 인접 구간 하나는 100m다. 목적지를 고른 뒤 A/D를 누르는 동안 횡스크롤 통로를 이동하고, 인카운터가 있는 100m 끝에서 자동으로 전투·사건으로 전환한다.
...
- 목적지 선택 뒤 A/D hold로 진행하는 100m 통로 이동과 인카운터 자동 정지
...
- 노드 선택은 이동 완료가 아니라 100m 통로 진입이다. A/D hold 진행률이 100m에 도달해야 시간 2분과 위치 변경을 정산하고 인카운터를 발생시킨다.
...
- 100m 통로 hold 이동, 인카운터 자동 정지, 고정 ALLY PLAN과 intent tooltip을 별도 screenshot으로 확인했다.
