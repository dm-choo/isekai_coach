...
> 작은 결계 안에서 혼자 깨어난 플레이어가 첫 위험을 직접 해결하고 봉인된 동료를 합류시킨다. 이후 인접한 미지의 좌표를 함께 탐사해 공간 규칙을 배우고, 동료 정책을 한 번 바꿔 알려진 통로를 위임하고, 주인공이 확보된 땅을 편입해 결계선과 다음 원정 능력이 실제로 확장되는 것을 본다.
...
- 초기 결계 안의 단독 각성, 첫 이동과 주인공 단독 학습 조우
- 첫 승리 뒤 world 오브젝트 상호작용으로 원거리 동료 1명 합류
