...
> **Superseded mechanic note (2026-08-17):** 이 검증 의제의 초기 `짧은 타격`/`광범위 공격` 및 `내려찍` 표현은 최신 Slice 1 canonical contract의 `제압`/`외침`/`내려찍기`와 피해·패턴 수치로 대체되었다. 테스트 목적과 관찰 질문은 보존하되 실행 시 최신 이름을 사용한다.
