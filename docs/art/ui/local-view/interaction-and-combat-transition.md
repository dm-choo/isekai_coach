---
title: Local Interaction and Combat Transition
status: accepted
last_updated: 2026-08-19
related:
  - ../../../gameplay/world/local-area-and-scouting.md
  - ../../../ux/views/local-view.md
  - ../combat-view/grid.md
---

# Local interaction and combat transition

- contextual prompt는 상호작용 대상 가까이에서만 간결하게 노출한다.
- 중앙 방과 네 경계 방은 실루엣과 출구 방향으로 구분하고, 통로 구간 이동 중에도 어느 월드 타일·방향에 있는지 잃지 않게 한다.
- 중앙 방 클리어 전의 미정찰 구간은 내용을 숨기고, 클리어 뒤에는 구간별 인카운터 종류와 위치를 월드 공간 표식으로 드러낸다.
- 정찰된 위협은 적 본체를 완전히 공개하지 않고 붉은 기척이나 오라로 암시한다.
- 방의 실제 방향 문을 선택하면 다음 방까지 이어지는 400m 통로 장면으로 전환한다. A/D hold 중 파티는 안정적인 screen anchor를 유지하고 배경·지면이 이동해 전진·후퇴를 전달하며, 100m 눈금, 목적지 문과 정찰된 위협 표식을 갱신한다.
- 100m 내부 경계마다 시간과 현재 판정 위치를 갱신한다. 저장된 인카운터 지점에서는 이동을 자동 정지하고 전투 또는 사건 전환을 시작하며, 해결 뒤 문을 다시 고르지 않고 같은 통로를 재개한다.
- 방 선택은 문 실루엣과 방향 화살표, 이동은 파티 모션과 거리선, 위험은 형태와 색을 함께 사용한다. 한국어 copy가 사라져도 1차 의미가 남아야 한다.
- 전투 진입 시 map UI를 끼우지 않고 현재 장면에서 짧은 감속·reveal 뒤 grid, Intent와 HUD가 조립되는 전환을 사용한다.
- 종료 시 같은 배경과 위치로 자연스럽게 해체한다.
- 전환은 로컬 공간과 전투 공간이 별도 세계처럼 보이지 않게 한다.
