...
Intent, ally action, target과 push result는 판독성을 우선한다.

- cell과 path의 논리 위치를 정확히 보존한다.
- enemy threat와 ally action signal이 겹쳐도 소유자와 순서를 구분할 수 있어야 한다.
- push result는 source, 이동 방향, old/new attack footprint를 읽을 수 있게 한다.
