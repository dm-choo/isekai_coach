...
- 관리자와 원거리 동료의 짧은 원정, 정책 수정, 새 조합, 보스와 결계 해제가 20~30분 골든 패스로 연결된다.
- 처음 보는 플레이어가 조작과 기본 전투 문법을 외부 설명 없이 발견한다.
- 원정 시작 상태와 결계 해제가 [승인된 premise](../narrative/index.md)와 충돌하지 않는다. 각성과 동료 합류 장면은 제출 경로 밖이다.
...
- 네 월드 타일에서 남은 HP, 세계 시간과 현재 policy가 결계 수호자 진입 상태에 그대로 유지된다.
