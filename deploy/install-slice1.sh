...
if ((EUID != 0)); then
  echo 'Run as root: sudo ./deploy/install-slice1.sh' >&2
  exit 1
fi
...
[[ -f "${dist_dir}/index.html" ]] || {
  echo "Missing validated slice build: ${dist_dir}/index.html" >&2
  exit 1
}
[[ -L "${release_root}/current" ]] || {
  echo "Missing shared release link: ${release_root}/current" >&2
  exit 1
}
[[ -f "${source_fragment}" ]] || {
  echo "Missing Caddy source: ${source_fragment}" >&2
  exit 1
}
...
case "${previous_release}" in
  ...
  *)
    echo "Refusing unexpected current release: ${previous_release}" >&2
    exit 1
    ;;
esac
...
rollback() {
  ...
  if ((fragment_installed == 1)); then
    cp -a -- "${caddy_backup}" "${caddy_fragment}"
  fi
...
for existing_path in "${previous_release}"/*; do
  ...
  case "${existing_name}" in
    slice1|.slice1-previous*) continue ;;
  esac
  cp -a -- "${existing_path}" "${release_dir}/"
done
shopt -u dotglob nullglob
install -d -o root -g caddy -m 0755 "${release_dir}/slice1"
cp -a "${dist_dir}/." "${release_dir}/slice1/"
