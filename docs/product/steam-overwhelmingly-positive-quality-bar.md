...
현재 공개본을 유료 정식판으로 그대로 출시하면 `압도적으로 긍정적`을 받을 후보가 아니다. 종합 상용 출시 준비도는 약 **24/100**이다. 정확한 리뷰 비율은 가격·상점 기대·대상 사용자와 실제 플레이 없이는 예측할 수 없지만, 현재 통과를 주장할 증거는 없다.
...
| 조작감·타격감 | 28 | 무음, 정지 PNG tween, 준비 전 입력 가능 |
| UX·온보딩 | 40 | 자동 bounds는 있으나 신규 사용자 증거 없음 |
...
| 슬라이스 기술 안정성 | 72 | 94 tests·누적 RC·공개 save/reload는 강점 |
...
```text
P21 첫 전투 readiness와 입력 신뢰
→ P22 첫 확장이 여는 두 번째 비지배 영토 선택
