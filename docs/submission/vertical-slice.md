...
- 사격 `#원거리공격`
- 밀치기 `#근거리공격 #넉백`
- 내려찍기 `#근거리공격`, AP 2, 피해 2

### Boss pattern sequence

- **1 — 제압:** 2칸 이동 뒤 BODY-anchor 1×1, 피해 6, 중단 불가. 수호자를 밀면 locked direction은 유지된 채 공격 원점과 범위가 함께 이동한다.
- **2 — 외침:** BODY-anchor 왼쪽 5칸 × 3개 행, 피해 2, 차징 중단 가능. 수호자가 차징 중 `#근거리공격`에 적중하면 현재 Intent만 취소되며 지속 스턴은 남지 않는다.
...
- `사격`은 같은 행 전방 3~6칸의 clear line을 요구하는 실제 투사체다. 최소 사거리 이전 전투원과 중간의 몸을 관통하지 않는다.
