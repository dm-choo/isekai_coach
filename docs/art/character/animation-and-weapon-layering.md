...
- `내려찍`: 관리자의 무기 wind-up·내려치는 순간, 수호자의 stun recoil과 중단된 `광범위 공격`을 보여준다.
- 보스 공격: `짧은 타격`과 `광범위 공격`은 wind-up shape와 affected area가 다르다. 임의의 고유 기술명은 붙이지 않는다.
...
- 무기만 교체 가능한 layer를 유지하되 이번 slice의 canonical public names는 `사격`, `밀치기`, `내려찍`이다.
