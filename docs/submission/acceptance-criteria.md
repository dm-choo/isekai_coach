...
- 텍스트를 읽지 않아도 시작 화면에서 홀로 있는 주인공, 초기 결계 안과 바깥으로 이어지는 진행 방향을 구분할 수 있다.
- 첫 조우는 주인공 단독으로 해결하고, 승리 뒤 주인공의 world 상호작용으로 원거리 동료가 합류한다.
- 합류 전에는 동료 HUD·prediction·policy가 없고 합류 뒤에만 해당 정보와 자동행동이 나타난다.
